...
2. **Token remapping.** `#CA` (jsonic's comma / member separator) is
   ...
   dropped to `nil`, because they only ever appear inside selectors and values,
...
5. **Node-building actions.** The AST is assembled entirely by
   grammar-local action refs (for example `@cssSheet`, `@cssRule`, `@cssDecl`,
...
a value, because the same characters can begin any of them. The engine allows
...
rule name alone, so it is stateless. The Go port additionally **cannot**
