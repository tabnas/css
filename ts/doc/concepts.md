...
to the next top-level `;` or `}` (for example `'1px solid #fff'`); a selector
...
the first body token. A comment seen mid-construct (for example between a
...
input, no rule ever runs, which is an engine convention. Any non-empty source,
