...
| [`ts/test/`](ts/test/) | TS tests (compiled to `dist-test/`): `css.test.ts` (AST parse cases), `debug-model.test.ts` (`@tabnas/debug` composition / model), `doc-examples.test.ts` (`// =>` assertions in README/doc fences), `perf.test.ts` (instance-reuse guard), `reworkcss.test.ts` + `cssparsing.test.ts` (third-party conformance), `leniency.test.ts` (jsonic-leak guard). |
...
| [`scripts/`](scripts/) | `fetch-css-parsing-tests.sh`, `fetch-reworkcss-tests.sh`, `fetch-suites.sh` — clone the third-party conformance corpora at PINNED commit SHAs into gitignored directories (never committed). `divergence-probe.sh` — TS/Go differential probe. |
| [`go/css_test.go`](go/css_test.go), [`go/perf_test.go`](go/perf_test.go), [`go/parity_test.go`](go/parity_test.go) | Go suite — the shared-fixture runner, the remaining in-language AST cases, and the perf guard. |
| [`go/conformance_test.go`](go/conformance_test.go), [`go/reworkcss_test.go`](go/reworkcss_test.go), [`go/cssparsing_test.go`](go/cssparsing_test.go) | Go third-party conformance: `TestMain` fetches the pinned corpora before any test; the runners mirror the TS ones exactly. |
...
## Conformance baseline (branch `conformance-2026-08`)

This repo measures itself against two **third-party** corpora, neither of
which is committed (see [`test/AGENTS.md`](test/AGENTS.md) and `scripts/`).
The numbers below were observed by running the suites, and both runtimes are
**identical** on every third-party case:

| Corpus | Half | TS | Go |
|---|---|---|---|
| reworkcss/css `ae6a6f9` | valid docs, exact AST | **37/46** | **37/46** |
| reworkcss/css `ae6a6f9` | valid docs, exact AST **+ position** | **37/46** | **37/46** |
| reworkcss/css `ae6a6f9` | must-reject inputs | **2/4** | **2/4** |
| reworkcss/css `ae6a6f9` | must-accept inputs | 2/2 | 2/2 |
| css-parsing-tests `203ce36` (CSS Syntax L3) | must-accept + correct kind sequence | **24/42** | **24/42** |
| css-parsing-tests `203ce36` (CSS Syntax L3) | must-reject | 12/12 | 12/12 |
| `test/spec/*.tsv` | shared fixtures | 72/72 | **62/72** |

Known gaps, all deliberately left RED — **do not** paper over them by editing
a corpus, a derivation, a census constant or an assertion:

- An unterminated `/*` is accepted as a comment node instead of being an
  error (reworkcss rejects `'/*'` and `'thing { color: red; } /* b {'`).
- Nine reworkcss documents produce the wrong AST: `custom-media`,
  `custom-media-linebreak`, `document-linebreak`, `empty`, `escapes`,
  `keyframes-advanced`, `paged-media`, `props`, `wtf`.
- 18 CSS Syntax L3 documents are rejected that the spec accepts — chiefly
  a qualified rule with an EMPTY prelude (`{ color: #aaa }`), CDO/CDC tokens
  (`<!--` / `-->`) between rules, and implicitly-closed blocks at EOF
  (`@foo { bar`). Note reworkcss deliberately rejects the empty-prelude case,
  so that one is a real disagreement between the two authorities, not a
  simple bug — see `test/AGENTS.md`.
- TS and Go diverge on comments inside at-rule preludes (see
  `test/spec/divergence.tsv`, 10 rows, red in Go by design).

**Leniency is NOT a problem in this plugin.** `new Tabnas().use(jsonic).use(Css)`
and `new Tabnas().use(Css)` classify every probe identically
(`ts/test/leniency.test.ts`, 34 probes) — jsonic's relaxed-JSON base does not
leak through, unlike `@tabnas/json5`.
