...
| [`scripts/divergence-probe.sh`](scripts/divergence-probe.sh), [`go/divergence_probe_test.go`](go/divergence_probe_test.go) | TS/Go differential probe over a deterministic generated corpus. A GATE (exits non-zero on divergence); `.github/workflows/divergence.yml` runs it, and is also where the superseded duplicate `probe` job lives. The Go half is inert unless `CSS_DUMP_IN`/`CSS_DUMP_OUT` are set. Runs with the default options only. |
...
One caveat a reader of a red check needs: `divergence.yml` defines a
SECOND job named `probe`, which predates the one above and fails at
`npm ci` because this repository tracks no lockfile. It is superseded and
should be deleted, which needs the `workflow` scope a session does not
have; it fails identically on `main`.
