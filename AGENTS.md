...
1. Bump all **three** version sites together — `ts/package.json`, `VERSION`
   in `ts/src/css.ts` and `const VERSION` in `go/css.go`. They are held
   equal by `ts/test/version.test.ts` and `go/version_test.go`.
...
6. Confirm `npm view @tabnas/css@$V version`, and **query both tags
   exactly**:
